...
A dashboard to monitor weather, air quality and flood trends, with a notification system.
...
and the dbname and username defined in the RDS definition in main.tf, and finally the host address which will be found on the AWS console. Then enter the db directory and enter these variables into a .env file. Create a venv and install requirements.txt.
Next run:
```
...
A file which creates a lambda handler which performs a get request for future climate predictions, from a given location, to an OpenMeteo api. It then inserts this data into the databases's future_readings table.
## extract-past/air_quality/extract_air_quality.py
A file which creates a lambda handler which performs a get request for past air quality, from a given location, to an OpenWeather api. It then inserts this data into the databases's historical air quality table.
## extract-past/weather/extract.py
A file which creates a lambda handler which performs a get request for historical climate data, from a given location, to an OpenMeteo api. It then inserts this data into the databases's historical_readings table.
